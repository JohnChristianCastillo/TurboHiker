...
public:
        ...
        std::vector<std::shared_ptr<GlobalBounds>> getLookAhead()
        {
                ...
                std::shared_ptr<GlobalBounds> frontalAura1 =
                    std::make_shared<GlobalBounds>(Position(aiPos.x, aiPos.y - aiDim.height), Dimentions(aiDim));
                std::shared_ptr<GlobalBounds> frontalAura2 =
                    std::make_shared<GlobalBounds>(Position(aiPos.x, aiPos.y - 2 * aiDim.height), Dimentions(aiDim));
                std::shared_ptr<GlobalBounds> frontalAura3 =
                    std::make_shared<GlobalBounds>(Position(aiPos.x, aiPos.y - 3 * aiDim.height), Dimentions(aiDim));
                std::shared_ptr<GlobalBounds> leftAura =
                    std::make_shared<GlobalBounds>(Position(aiPos.x - aiDim.width / 2, aiPos.y), Dimentions(aiDim));
                std::shared_ptr<GlobalBounds> rightAura =
                    std::make_shared<GlobalBounds>(Position(aiPos.x + aiDim.width / 2, aiPos.y), Dimentions(aiDim));
                std::shared_ptr<GlobalBounds> upRightAura = std::make_shared<GlobalBounds>(
                    Position(aiPos.x + aiDim.width / 2, aiPos.y - aiDim.height), Dimentions(aiDim));
